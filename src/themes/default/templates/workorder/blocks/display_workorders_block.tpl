...
<table class="olotable" width="100%" border="0" cellpadding="4" cellspacing="0">
    ...
    {section name=w loop=$display_workorders.records}        
        <tr class="row1" onmouseover="this.className='row2';" onmouseout="this.className='row1';"{if $display_workorders.records[w].workorder_status != 'deleted'} onDblClick="window.location='index.php?component=workorder&page_tpl={if $display_workorders.records[w].workorder_is_closed}details{else}edit{/if}&workorder_id={$display_workorders.records[w].workorder_id}';"{/if}>
            ...
            <td class="olotd4"><a href="index.php?component=invoice&page_tpl=details&invoice_id={$display_workorders.records[w].invoice_id}">{$display_workorders.records[w].invoice_id}</a></td>
            <td class="olotd4"> {$display_workorders.records[w].workorder_opened_on|date_format:$date_format}</td>
            <td class="olotd4">{$display_workorders.records[w].workorder_closed_on|date_format:$date_format}</td>
            <td class="olotd4" nowrap>
                {if $display_workorders.records[w].workorder_status != 'deleted'}
                    <img src="{$theme_images_dir}icons/16x16/view.gif" border="0" onMouseOver="ddrivetip('<b><center>{t}Client Info{/t}</b></center><hr><b>{t}Contact{/t}:</b> {$display_workorders.records[w].client_first_name} {$display_workorders.records[w].client_last_name}<br><b>{t}Phone{/t}: </b>{$display_workorders.records[w].client_phone}<br><b>{t}Mobile{/t}: </b>{$display_workorders.records[w].client_mobile_phone}<br><b>{t}Fax{/t}: </b>{$display_workorders.records[w].client_phone}<br><b>{t}Address{/t}: </b><br>{$display_workorders.records[w].client_address|nl2br|regex_replace:"/[\r\t\n]/":" "}<br>{$display_workorders.records[w].client_city}<br>{$display_workorders.records[w].client_state}<br>{$display_workorders.records[w].client_zip}<br>{$display_workorders.records[w].client_country}');" onMouseOut="hideddrivetip();">
                    <a class="link1" href="index.php?component=client&page_tpl=details&client_id={$display_workorders.records[w].client_id}">{$display_workorders.records[w].client_display_name}</a>
                {/if}
            </td>
            <td class="olotd4" nowrap>
                {if $display_workorders.records[w].workorder_scope}
                    {$display_workorders.records[w].workorder_scope|truncate:20:"..."|htmlentities|regex_replace:"/[\t\r\n']/":" "}&nbsp;
                    <img src="{$theme_images_dir}icons/16x16/view.gif" border="0" alt="" onMouseOver="ddrivetip('<div><strong>{t}Scope{/t}</strong></div><hr><div>{$display_workorders.records[w].workorder_scope|htmlentities|regex_replace:"/[\t\r\n']/":" "}</div>');" onMouseOut="hideddrivetip();">
                 {/if}
            </td>
