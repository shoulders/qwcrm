...
<table class="olotable" width="100%" border="0" cellpadding="5" cellspacing="0">
    ...
    {section name=i loop=$display_creditnotes.records}
        <tr class="row1" onmouseover="this.className='row2';" onmouseout="this.className='row1';"{if $display_creditnotes.records[i].status != 'deleted'} onDblClick="window.location='index.php?component=creditnote&page_tpl={if $display_creditnotes.records[i].is_closed}details{else}edit{/if}&creditnote_id={$display_creditnotes.records[i].creditnote_id}';"{/if}>
            ...
            <td class="olotd4" nowrap>
                {if $display_creditnotes.records[i].redemptions|voucher_redemptions}
                    <img src="{$theme_images_dir}icons/16x16/view.gif" border="0" alt="" onMouseOver="ddrivetip('<div><strong>{t}Redemptions{/t}</strong></div><hr><div>{$display_creditnotes.records[i].redemptions|voucher_redemptions|htmlentities|regex_replace:"/[\t\r\n']/":" "}</div>');" onMouseOut="hideddrivetip();">
                {/if}
            </td>
            <td class="olotd4" nowrap>
                {if $display_creditnotes.records[i].additional_info|creditnote_addinfo_display}
                    <img src="{$theme_images_dir}icons/16x16/view.gif" border="0" alt="" onMouseOver="ddrivetip('<div><strong>{t}Additional Info{/t}</strong></div><hr><div>{$display_creditnotes.records[i].additional_info|creditnote_addinfo_display|htmlentities|regex_replace:"/[\t\r\n']/":" "}</div>');" onMouseOut="hideddrivetip();">
                {/if}
            </td>
        </tr>
    {/section}
