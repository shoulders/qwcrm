...
<table class="olotable" width="100%" border="0" cellpadding="5" cellspacing="0">
    ...
    {section name=r loop=$display_cronjobs}                                                            
        <!-- This allows double clicking on a row and opens the corresponding cron view details -->
        <tr class="row1" onmouseover="this.className='row2';" onmouseout="this.className='row1';" onDblClick="window.location='index.php?component=cronjob&page_tpl=details&cronjob_id={$display_cronjobs[r].cronjob_id}';">                                                                
            <td class="olotd4" nowrap><a href="index.php?component=cronjob&page_tpl=details&cronjob_id={$display_cronjobs[r].cronjob_id}">{$display_cronjobs[r].cronjob_id}</a></td>
            <td class="olotd4" nowrap>{$display_cronjobs[r].name}</td>
            <td class="olotd4" nowrap>{if $display_cronjobs[r].description != ''}<img src="{$theme_images_dir}icons/16x16/view.gif" border="0" alt="" onMouseOver="ddrivetip('<div><strong>{t}Description{/t}</strong></div><hr><div>{$display_cronjobs[r].description|htmlentities|regex_replace:"/[\t\r\n']/":" "}</div>');" onMouseOut="hideddrivetip();">{/if}</td>                                                            
            <td class="olotd4" nowrap>{if $display_cronjobs[r].active == 1}{t}Yes{/t}{else}{t}No{/t}{/if}</td>
            <td class="olotd4" nowrap>{if $display_cronjobs[r].pseudo_allowed == 1}{t}Yes{/t}{else}{t}No{/t}{/if}</td>             
            <td class="olotd4" nowrap>{$display_cronjobs[r].last_run_time|date_format:$date_format} {$display_cronjobs[r].last_run_time|date_format:'H:i:s'}</td>   
            <td class="olotd4">{if $display_cronjobs[r].last_run_status == 1}{t}Success{/t}{else}{t}Failed{/t}{/if}</td>
            <td class="olotd4">{if $display_cronjobs[r].locked == 1}{t}Yes{/t}{else}{t}No{/t}{/if}</td> 
            <td class="olotd4" nowrap>{$display_cronjobs[r].minute}</td>
            ...
            <td class="olotd4" nowrap>
                <a href="index.php?component=cronjob&page_tpl=details&cronjob_id={$display_cronjobs[r].cronjob_id}">
                ...
                <a href="index.php?component=cronjob&page_tpl=edit&cronjob_id={$display_cronjobs[r].cronjob_id}">
