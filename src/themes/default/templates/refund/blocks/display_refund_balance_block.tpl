...
<table width="100%" cellpadding="4" cellspacing="0" border="0" class="olotable">
    <tr class="olotd4">
        ...
        {if $refund_details.tax_system != 'none'}
            <td class="olohead"><b>{if '/^vat_/'|preg_match:$refund_details.tax_system}{t}VAT{/t}{else}{t}Sales Tax{/t} (@ {$refund_details.sales_tax_rate|string_format:"%.2f"}%){/if}</b></td>
        {/if}
    ...
    <tr class="olotd4">
        ...
        {if $refund_details.tax_system != 'none'}<td class="olotd4">{$currency_sym}{$refund_details.unit_tax|string_format:"%.2f"}</td>{/if}
