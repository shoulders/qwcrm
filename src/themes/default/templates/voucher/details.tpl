            ...
            <table width="100%" cellpadding="4" cellspacing="0" border="0" class="olotable">
                                ...
                                <td valign="top" width="50%">
                                    <table cellpadding="0" cellspacing="0" border="0" width="100%">
                                        ...
                                        {if $voucher_details.tax_system != 'no_tax'}
                                            <tr>
                                                <td><b>{t}Net{/t}</b></td>
                                                <td>{$currency_symbol}{$voucher_details.unit_net|string_format:"%.2f"}</td>
                                            </tr>
                                            <tr>
                                                <td><b>{if '/^vat_/'|preg_match:$voucher_details.tax_system}{t}VAT{/t}{else}{t}Sales Tax{/t}{/if}</b></td>
                                                <td>{$currency_symbol}{$voucher_details.unit_tax|string_format:"%.2f"}</td>
                                            </tr>
                                        {/if}
                                        <tr>
                                            <td><b>{t}Gross{/t}</b></td>
                                            <td>{$currency_symbol}{$voucher_details.unit_gross|string_format:"%.2f"}</td>
                                        </tr>
                                        <tr>
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <br>
                        {if $allowed_to_edit}
                            <button type="button" onclick="window.location.href='index.php?component=voucher&page_tpl=edit&voucher_id={$voucher_details.voucher_id}';">{t}Edit{/t}</button>
                        {/if}
                        <button type="button" onclick="window.location.href='index.php?component=voucher&page_tpl=status&voucher_id={$voucher_details.voucher_id}';">{t}Status{/t}</button>
                    </td>
                </tr>
            </table>
        </td>
