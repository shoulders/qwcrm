...
<table class="olotable" width="100%" border="0" cellpadding="5" cellspacing="0">
    ...
    {section name=i loop=$display_invoices}
        <tr onmouseover="this.className='row2';" onmouseout="this.className='row1';" onDblClick="window.location='index.php?page=invoice:edit&invoice_id={$display_invoices[i].invoice_id}';" class="row1">
            <td class="olotd4" nowrap><a href="index.php?page=invoice:edit&invoice_id={$display_invoices[i].invoice_id}">{$display_invoices[i].invoice_id}</a></td>
            <td class="olotd4" nowrap><a href="index.php?page=workorder:details&workorder_id={$display_invoices[i].workorder_id}">{$display_invoices[i].workorder_id}</a></td>
            ...
            <td class="olotd4" nowrap><img src="{$theme_images_dir}icons/16x16/view.gif" border="0" onMouseOver="ddrivetip('<b><center>{t}Contact Info{/t}</b></center><hr><b>{t}Contact{/t}: </b>{$display_invoices[i].customer_first_name} {$display_invoices[i].customer_last_name}<br><b>{t}Phone{/t}: </b>{$display_invoices[i].customer_phone}<br><b>{t}Mobile{/t}: </b>{$display_invoices[i].customer_mobile_phone}<br><b>{t}Fax{/t}: </b>{$display_invoices[i].customer_fax}');" onMouseOut="hideddrivetip();"><a href="index.php?page=customer:details&customer_id={$display_invoices[i].customer_id}"> {$display_invoices[i].customer_display_name}</a></td>
            <td class="olotd4" nowrap><img src="{$theme_images_dir}icons/16x16/view.gif" border="0" onMouseOver="ddrivetip('<center><b>{t}Contact{/t}</b></center><hr><b>{t}Phone{/t}: </b>{$display_invoices[i].employee_work_primary_phone}<br><b>{t}Mobile{/t}: </b>{$display_invoices[i].employee_work_mobile_phone}<br><b>{t}Personal{/t}: </b>{$display_invoices[i].employee_home_mobile_phone}');" onMouseOut="hideddrivetip();"><a  href="index.php?page=user:details&user_id={$display_invoices[i].employee_id}"> {$display_invoices[i].employee_display_name}</td>
