        ...
        <tr class="row1" onmouseover="this.className='row2';" onmouseout="this.className='row1';"{if $display_invoices.records[i].status != 'deleted'} onDblClick="window.location='index.php?component=invoice&page_tpl={if $display_invoices.records[i].closed_on}details{else}edit{/if}&invoice_id={$display_invoices.records[i].invoice_id}';"{/if}>
            ...
            <td class="olotd4" nowrap>
                {if $display_invoices.records[i].scope}
                    <img src="{$theme_images_dir}icons/16x16/view.gif" border="0" alt="" onMouseOver="ddrivetip('<div><strong>{t}Work order{/t} {t}Scope{/t}</strong></div><hr><div>{$display_invoices.records[i].scope|htmlentities|regex_replace:"/\|\|\|/":"<br>"|regex_replace:"/[\t\r\n']/":" "}</div>');" onMouseOut="hideddrivetip();">
                {/if}
            </td>
            <td class="olotd4" nowrap>
            ...
            {if $qw_tax_system != 'no_tax'}
                <td class="olotd4" nowrap>{$currency_symbol}{$display_invoices.records[i].unit_net|string_format:"%.2f"}</td>
                <td class="olotd4" nowrap>{$currency_symbol}{$display_invoices.records[i].unit_tax|string_format:"%.2f"}</td>
            {/if}
            <td class="olotd4" nowrap>{$currency_symbol}{$display_invoices.records[i].unit_gross|string_format:"%.2f"}</td>
            <td class="olotd4" nowrap>{$currency_symbol}{$display_invoices.records[i].balance|string_format:"%.2f"}</td>
            <td class="olotd4" nowrap>
