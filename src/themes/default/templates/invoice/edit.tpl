...
<script>
    ...
    function applyDiscountRateButton() {
        ...
        if (clientDiscountRate < 0 || clientDiscountRate > 100) {
            alert("{t}The client discount rate must be within the range of 0 and 100{/t}");
        } else {
            refreshPage(true);
        }
    }
    ...
    function refreshTotals(cleanUserInputs = false, applyDiscountRate = false) {
        ...
        $('.invoice_item_row').each(function() {
            ...
            // Validate Unit Qty, Unit Net and Unit Discount input fields and blocks form submission
            let unitQtyInput      = $(this).find("input[id$='\\[unit_qty\\]']")[0];
            let unitNetInput      = $(this).find("input[id$='\\[unit_net\\]']")[0];
            let unitDiscountInput = $(this).find("input[id$='\\[unit_discount\\]']")[0];
            ...
            if(rowUnitDiscount < 0) {
                unitDiscountInput.setCustomValidity("{t}Discount cannot be negative.{/t}");
            } else if(rowUnitDiscount > rowUnitNet && !(rowUnitDiscount === 0 && rowUnitNet === 0)) {
                unitDiscountInput.setCustomValidity("{t}Discount cannot be more than Net.{/t}");
            } else {
                unitDiscountInput.setCustomValidity("");
            }
                                        ...
                                        <table width="100%" cellpadding="4" cellspacing="0" border="0" class="olotable">
                                            ...
                                            <tr{if !$invoice_details.client_id} hidden{/if}>
                                                ...
                                                <td colspan="6" valign="top" align="left">
                                                    <input type="number" class="olotd4" size="6" id="client_discount_rate" value="{$client_details.discount_rate|string_format:"%.2f"}" min="0" max="100" step="0.01"> %
                                            ...
                                            <tr>
                                                <td class="menutd2">
                                                    ...
                                                    {if $invoice_details.status == 'unpaid' || $invoice_details.status == 'partially_paid'}
                                                        <button type="button" class="userButton" onclick="window.location.href='index.php?component=payment&page_tpl=new&type=invoice&invoice_id={$invoice_details.invoice_id}';">{t}Record Payment{/t}</button>
                                                    {/if}
                                                ...
                                                <td class="menutd2">
                                                    <table id="invoice_items" width="100%" cellpadding="3" cellspacing="0" border="0" class="olotable">
                                                        ...
                                                        <tr id="dummy_invoice_items_row_iteration" class="dummy_invoice_item_row olotd4" style="display: none;">
                                                            ...
                                                            <td class="salesTaxSystem" align="right" hidden><input id="qform[invoice_items][iteration][sales_tax_exempt]" name="qform[invoice_items][iteration][sales_tax_exempt]" type="checkbox" value="1" disabled></td>
                                                            <td align="right">
                                        ...
                                        <table width="100%"  cellpadding="3" cellspacing="0" border="0">
                                            <tr>
                                                <td align="left" valign="top" width="25%">
                                                    ...
                                                    <button type="submit" name="submit" value="submitandapprove" onclick="return confirm('{t}Are you sure you want to submit and approve this invoice?{/t}');">{t}Submit and Approve{/t}</button>
