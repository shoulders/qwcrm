...
<script>
    ...
    function createNewTableRow() {
        ...
        // Prevent negative amounts being generated by a discount
        /*$('#qform\\[invoice_items\\]\\['+iteration+'\\]\\[unit_discount\\]').off("change").on("change", function() {
            let discountValue = +$(this).val();
            if (discountValue > $('#qform\\[invoice_items\\]\\['+iteration+'\\]\\[unit_net\\]').val()) {
                this.setCustomValidity("Discount cannot be greater than 100.");
                alert('chickein');
            } else {
