                                            ...
                                            <form method="post" action="index.php?component=expense&page_tpl=edit&expense_id={$expense_details.expense_id}" name="edit_expense" id="edit_expense" class="amend_values">
                                                <table width="100%" cellpadding="2" cellspacing="2" border="0">                                                    
                                                        ...
                                                        <td>
                                                            <select id="vat_tax_code" name="qform[vat_tax_code]" class="olotd5">
                                                                ...
                                                                {section name=s loop=$vat_tax_codes}    
                                                                    <option value="{$vat_tax_codes[s].tax_key}" data-rate="{$vat_tax_codes[s].rate}"{if $expense_details.vat_tax_code == $vat_tax_codes[s].tax_key} selected{/if} data-only_gross="{$vat_tax_codes[s].only_gross}">{$vat_tax_codes[s].tax_key} - {t}{$vat_tax_codes[s].display_name}{/t} @ {$vat_tax_codes[s].rate|string_format:"%.2f"}%</option>
                                                                {/section} 
                                                            </select>                                                            
                                                        </td>
                                                    </tr>
                                                    <tr{if !'/^vat_/'|preg_match:$expense_details.tax_system} hidden{/if}>
                                                        ...
                                                        <td><input id="unit_tax_rate" name="qform[unit_tax_rate]" class="olotd5" size="4" value="{$expense_details.unit_tax_rate}" type="text" maxlength="5" pattern="{literal}^[0-9]{0,2}(\.[0-9]{0,2})?${/literal}" required onkeydown="return onlyNumberPeriod(event);" onkeyup="calculateTotals('unit_tax_rate');" readonly/><b>%</b></td>
                                                    </tr>
                                                    <tr{if !'/^vat_/'|preg_match:$expense_details.tax_system} hidden{/if}>
