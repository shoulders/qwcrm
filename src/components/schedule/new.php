...
if(isset(\CMSApplication::$VAR['submit'])) {    
    ...
    if (!insert_schedule(\CMSApplication::$VAR['qform'])) {        
       ...
       $smarty->assign('schedule_details', \CMSApplication::$VAR['qform']);
        ...
        $start_year            = date('Y', date_to_timestamp(\CMSApplication::$VAR['qform']['start_date'])  );
        $start_month           = date('m', date_to_timestamp(\CMSApplication::$VAR['qform']['start_date'])  );
        $start_day             = date('d', date_to_timestamp(\CMSApplication::$VAR['qform']['start_date'])  );
    
        // Load the schedule day with the newly submitted schedule item
        force_page('schedule', 'day', 'start_year='.$start_year.'&start_month='.$start_month.'&start_day='.$start_day.'&employee_id='.\CMSApplication::$VAR['qform']['employee_id'].'&workorder_id='.\CMSApplication::$VAR['qform']['workorder_id'].'&msg_success='._gettext("Schedule Successfully Created"));
...
// If new schedule form is intially loaded, load schedule item from the database and assign
    ...
    $schedule_details['start_date']     = convert_year_month_day_to_date(\CMSApplication::$VAR['start_year'], \CMSApplication::$VAR['start_month'], \CMSApplication::$VAR['start_day']);
    $schedule_details['start_time']     = \CMSApplication::$VAR['start_time'];
    $schedule_details['end_date']       = convert_year_month_day_to_date(\CMSApplication::$VAR['start_year'], \CMSApplication::$VAR['start_month'], \CMSApplication::$VAR['start_day']);
