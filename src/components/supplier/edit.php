...
if(isset(\CMSApplication::$VAR['submit'])) {    
    ...
    update_supplier(\CMSApplication::$VAR['qform']);
    
    // load the supplier details apge
    force_page('supplier', 'details&supplier_id='.\CMSApplication::$VAR['supplier_id'], 'msg_success='._gettext("Supplier updated successfully."));     
    
} else {
    
    // Check if supplier can be edited
    if(!check_supplier_can_be_edited(\CMSApplication::$VAR['supplier_id'])) {
        systemMessagesWrite('danger', _gettext("You cannot edit this supplier because its status does not allow it."));
        force_page('supplier', 'details&supplier_id='.\CMSApplication::$VAR['supplier_id']);
    }
