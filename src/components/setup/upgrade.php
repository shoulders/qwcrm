...
if(!isset(\CMSApplication::$VAR['stage']) || \CMSApplication::$VAR['stage'] == 'database_connection') {
    ...
    if(isset(\CMSApplication::$VAR['submit']) && \CMSApplication::$VAR['submit'] == 'database_connection') {
        ...
        if(Setup::$split_database_upgrade) {
            $message = _gettext("The upgrade procedure has been split into multiple parts to prevent server timeouts.").'<br>';
            $message .= _gettext("Follow this process through until the upgrade is complete.").'<br>';
            $message .= _gettext("The final QWcrm version will be").': '.QWCRM_VERSION;
        ...
        if($this->app->db->isConnected()) {
            ...
            $message = _gettext("Connected successfully to the database with the supplied credentials from the config file.");
            $this->app->system->variables->systemMessagesWrite('success', $message);
            $this->app->components->setup->writeRecordToSetupLog('upgrade', $message);
        } else {
            $this->app->smarty->assign('enable_next', false);
            $message = _gettext("Failed to connect to the database with the supplied credentials. Check your config file.");
...
if(\CMSApplication::$VAR['stage'] == 'database_upgrade') {
    ...
    if(isset(\CMSApplication::$VAR['submit']) && \CMSApplication::$VAR['submit'] == 'database_upgrade') {
        ...
        if(!Setup::$setup_error_flag) {
            $message = _gettext("The database upgraded successfully.");
            $this->app->system->variables->systemMessagesWrite('success', $message);
            $this->app->components->setup->writeRecordToSetupLog('upgrade', $message);
        ...
        // Load the results page with the error message
           ...
           $message = _gettext("The database failed to upgrade.");
           $this->app->system->variables->systemMessagesWrite('danger', $message);
           $this->app->components->setup->writeRecordToSetupLog('upgrade', $message);
           \CMSApplication::$VAR['stage'] = 'database_upgrade_results';
...
if(\CMSApplication::$VAR['stage'] == 'database_upgrade_results') {
    ...
    if(isset(\CMSApplication::$VAR['submit']) && \CMSApplication::$VAR['submit'] == 'database_upgrade_results') {
            ...
            $message = _gettext("The QWcrm upgrade process has completed successfully.");
            $this->app->components->setup->writeRecordToSetupLog('upgrade', $message);
            $this->app->system->variables->systemMessagesWrite('success', $message);
    ...
    // Load the page
        ...
        if(Setup::$split_database_upgrade) {
            $message = _gettext("This QWcrm upgrade `Part` has completed successfully.");
            $this->app->components->setup->writeRecordToSetupLog('upgrade', $message);
            $this->app->system->variables->systemMessagesWrite('success', $message);
