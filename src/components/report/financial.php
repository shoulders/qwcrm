...
if(isset(\CMSApplication::$VAR['start_date'], \CMSApplication::$VAR['end_date']))
{
    ...
    $invoice_stats = $this->app->components->report->invoiceGetStats('all', $start_date, $end_date, QW_TAX_SYSTEM);
    $voucher_stats = $this->app->components->report->voucherGetStats('all', $start_date, $end_date, QW_TAX_SYSTEM);
    $refund_stats = $this->app->components->report->getRefundsStats('all', $start_date, $end_date, QW_TAX_SYSTEM);
    $expense_stats = $this->app->components->report->expenseGetStats('all', $start_date, $end_date, QW_TAX_SYSTEM);
    $otherincome_stats = $this->app->components->report->otherincomeGetStats('all', $start_date, $end_date, QW_TAX_SYSTEM);
    $payment_stats = $this->app->components->report->paymentGetStats('all', $start_date, $end_date, QW_TAX_SYSTEM);

    // Assign stats to Template variables
    $this->app->smarty->assign('client_stats', $this->app->components->report->clientGetStats('current', $start_date, $end_date));
    $this->app->smarty->assign('workorder_stats', $this->app->components->report->workorderGetStats('historic', $start_date, $end_date));
