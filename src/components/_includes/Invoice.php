...
defined('_QWEXEC') or die;
    ...
    public function display_invoices($order_by, $direction, $use_pages = false, $records_per_page = null, $page_no = null, $search_category = null, $search_term = null, $status = null, $employee_id = null, $client_id = null) {
        ...
        if($use_pages) {
            ...
            if(!$rs = $this->app->db->Execute($sql)) {
                $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to count the matching Invoice records."));
            } else {        
        ...
        if(!$rs = $this->app->db->Execute($sql)) {
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to return the matching Invoice records."));
        } else {
    ...
    */
    ...
    public function get_invoice_details($invoice_id, $item = null) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get invoice details."));
        } else {
    ...
    public function get_invoice_labour_items($invoice_id) {
        ...
        if(!$rs = $this->app->db->Execute($sql)) {
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get invoice labour items."));
        } else {
    ...
    public function get_invoice_labour_item_details($invoice_labour_id, $item = null) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get invoice labour item details."));
        } else {
    ...
    public function get_invoice_parts_items($invoice_id) {
        ...
        if(!$rs = $this->app->db->Execute($sql)) {
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get invoice parts items."));
        } else {
    ...
    public function get_invoice_parts_item_details($invoice_parts_id, $item = null) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get invoice parts item details."));
        } else {
    ...
    public function get_invoice_prefill_items($type = null, $status = null) {
        ...
        if(!$rs = $this->app->db->Execute($sql)) {
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get the invoice prefill items for the selected status."));
        } else {
    ...
    public function get_invoice_statuses($restricted_statuses = false) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get invoice statuses."));
        } else {
    ...
    public function get_invoice_status_display_name($status_key) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get the invoice status display name."));
        } else {
    ...
    public function update_invoice_full($qform, $doNotLog = false) {
        ...
        if(!$rs = $this->app->db->execute($sql)) {        
            ...
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to update the invoice."));
    ...
    public function update_invoice_prefill_item($qform) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to update an invoice labour rates item."));

        } else {

            // Log activity        
            $this->app->system->general->write_record_to_activity_log(_gettext("The Invoice Prefill Item").' '.$qform['invoice_prefill_id'].' '._gettext("was updated by").' '.$this->app->user->login_display_name.'.');    
    ...
    public function update_invoice_status($invoice_id, $new_status) {
        ...
        if(!$rs = $this->app->db->Execute($sql)) {
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to update an Invoice Status."));
    ...
    public function update_invoice_closed_status($invoice_id, $new_closed_status) {
        ...
        if(!$rs = $this->app->db->Execute($sql)) {
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to update an invoice Closed status."));
        }
    ...
    public function delete_invoice($invoice_id) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), null, _gettext("Failed to delete the invoice."));
        } else {
    ...
    public function delete_invoice_labour_items($invoice_id) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to delete all of an invoice's labour items."));
        } else {
    ...
    public function delete_invoice_parts_items($invoice_id) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to delete all of an invoice's parts items."));
        } else {
    ...
    public function delete_invoice_prefill_item($invoice_prefill_id) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to delete an invoice prefill item."));

        } else {

            // Log activity        
            $this->app->system->general->write_record_to_activity_log(_gettext("The Invoice Prefill Item").' '.$invoice_prefill_id.' '._gettext("was deleted by").' '.$this->app->user->login_display_name.'.');
    ...
    public function recalculate_invoice_totals($invoice_id) {
        ...
        if(!$rs = $this->app->db->execute($sql)){        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to recalculate the invoice totals."));
        } else {
    ...
    public function upload_invoice_prefill_items_csv($empty_prefill_items_table) {
        ...
        if ((($_FILES['invoice_prefill_csv']['type'] == 'text/csv'))            
            ...
            // If no errors then proceed to processing the data
                ...
                if($empty_prefill_items_table) {
                    ...
                    if(!$rs = $this->app->db->execute($sql)) {
                        $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to empty the prefill items table."));
                    }
                }
                ...
                while (($data = fgetcsv($handle)) !== FALSE) {
                    ...
                    if(!$rs = $this->app->db->execute($sql)) {
                        $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to insert the new prefill items into the database."));
                    }
                ...
                // Log activity        
                $this->app->system->general->write_record_to_activity_log(_gettext("Invoice Prefill Items were uploaded via csv by").' '.$this->app->user->login_display_name.'.'); 
        ...
        // If file is invalid then load the error page  
             ...
             */
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to update the invoice labour rates because the submitted file was invalid."));
    ...
    public function export_invoice_prefill_items_csv() {
        ...
        if(!$rs = $this->app->db->Execute($sql)) {        
            $this->app->system->page->force_error_page('database', __FILE__, __FUNCTION__, $this->app->db->ErrorMsg(), $sql, _gettext("Failed to get invoice prefill items from the database."));
        } else {        
            ...
            // Log activity        
            $this->app->system->general->write_record_to_activity_log(_gettext("Invoice Prefill Items were exported by").' '.$this->app->user->login_display_name.'.');
    ...
    ##########################################################
     ...
     public function check_invoice_status_can_be_changed($invoice_id) {
        ...
        if(!$this->app->components->voucher->check_invoice_vouchers_allow_editing($invoice_id)) {
    ...
    public function check_invoice_can_be_refunded($invoice_id) {
        ...
        if(!$this->app->components->voucher->check_invoice_vouchers_allow_refunding($invoice_id)) {
            $this->app->system->variables->systemMessagesWrite('danger', _gettext("The invoice cannot be refunded because of Vouchers on it prevent this."));
            return false;
        }    
    ...
    public function check_invoice_can_be_cancelled($invoice_id) {
        ...
        if(!$this->app->components->voucher->check_invoice_vouchers_allow_cancellation($invoice_id)) {
    ...
    public function check_invoice_can_be_deleted($invoice_id) {
        ...
        */
        ...
        // Does the invoice have any Vouchers preventing refunding the invoice (i.e. any that have been used)
        if(!$this->app->components->voucher->check_invoice_vouchers_allow_deletion($invoice_id)) {
    ...
    public function check_invoice_can_have_refund_deleted($invoice_id) {
        ...
        // Does the invoice have any Vouchers preventing refunding the invoice (i.e. any that have been used)
        if(!$this->app->components->voucher->check_invoice_vouchers_allow_refund_deletion($invoice_id)) {
            $this->app->system->variables->systemMessagesWrite('danger', _gettext("The invoice cannot be refunded because of Vouchers on it prevent this."));
            return false;
        }    
    ...
    public function check_invoice_can_have_refund_cancelled($invoice_id) {
        ...
        /* Is partially paid
        if($invoice_details['status'] == 'partially_paid') {
            $this->app->system->variables->systemMessagesWrite('danger', _gettext("This invoice cannot have its refunded cancelled because the invoice is partially paid."));
            return false;
        }

        // Is cancelled
        if($invoice_details['status'] == 'cancelled') {
            $this->app->system->variables->systemMessagesWrite('danger', _gettext("This invoice cannot have its refunded cancelled because the invoice has been cancelled."));
            return false;        
        }

        // Is deleted
        if($invoice_details['status'] == 'deleted') {
            $this->app->system->variables->systemMessagesWrite('danger', _gettext("This invoice cannot have its refunded cancelled because the invoice has been deleted."));
            return false;        
        }*/

        // Is not refunded (fall back)
        if($invoice_details['status'] != 'refunded') {
            $this->app->system->variables->systemMessagesWrite('danger', _gettext("This invoice cannot have its refunded cancelled because it is not refunded."));
            return false;        
        }

        // Has no payments (Fallback - is currently not needed because of statuses, but it might be used for information reporting later)
        if(!$this->app->components->report->count_payments(null, null, 'date', null, null, 'invoice', null, null, null, $invoice_id)) { 
            $this->app->system->variables->systemMessagesWrite('danger', _gettext("This invoice cannot have its refunded cancelled because the invoice has no payments."));
            return false;        
        }

        // Does the invoice have any Vouchers preventing refunding the invoice (i.e. any that have been used)
        if(!$this->app->components->voucher->check_invoice_vouchers_allow_refund_cancellation($invoice_id)) {
            $this->app->system->variables->systemMessagesWrite('danger', _gettext("This invoice cannot have its refunded cancelled because of Vouchers on it prevent this."));
            return false;
        }    
