...
class Report extends Components {
    ...
    public function count_clients($start_date = null, $end_date = null, $status = null) { 
        ...
        if($start_date && $end_date) {
            $whereTheseRecords .= " AND ".PRFX."client_records.opened_on >= ".$this->db->qstr($start_date)." AND ".PRFX."client_records.opened_on <= ".$this->db->qstr($end_date.' 23:59:59');
        }

        // Restrict by Status
        if($status) {        
            $whereTheseRecords .= " AND ".PRFX."client_records.active= ".$this->db->qstr($status);            
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count the number of clients."));
        } else {
    ...
    public function count_workorders($start_date = null, $end_date = null, $date_type = null, $status = null, $employee_id = null, $client_id = null) {
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."workorder_records.employee_id=".$this->db->qstr($employee_id);
        }

        // Filter by Client
        if($client_id) {
            $whereTheseRecords .= " AND ".PRFX."workorder_records.client_id=".$this->db->qstr($client_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count Work Orders for the defined status."));
    ...
    public function workorder_build_filter_by_date($start_date = null, $end_date = null, $date_type = null) {
        ...
        if($start_date && $end_date && $date_type) {
            if($date_type == 'opened_on') {
                $whereTheseRecords .= " AND ".PRFX."workorder_records.opened_on >= ".$this->db->qstr($start_date)." AND ".PRFX."workorder_records.opened_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif($date_type == 'closed_on') {       
                $whereTheseRecords .= " AND ".PRFX."workorder_records.closed_on >= ".$this->db->qstr($start_date)." AND ".PRFX."workorder_records.closed_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif($date_type == 'last_active') {       
                $whereTheseRecords .= " AND ".PRFX."workorder_records.last_active >= ".$this->db->qstr($start_date)." AND ".PRFX."workorder_records.last_active <= ".$this->db->qstr($end_date.' 23:59:59');
            }
        }
    ...
    public function count_schedules($workorder_id = null) {
        ...
        if($workorder_id) {
            $whereTheseRecords .= " AND ".PRFX."schedule_records.workorder_id=".$this->db->qstr($workorder_id);
        }    
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count schedule items for the specified Work Order."));
    ...
    public function count_invoices($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $status = null, $employee_id = null, $client_id = null) {   
        ...
        if($tax_system) {
            $whereTheseRecords .= " AND ".PRFX."invoice_records.tax_system=".$this->db->qstr($tax_system);
        }
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."invoice_records.employee_id=".$this->db->qstr($employee_id);
        }

        // Filter by Client
        if($client_id) {
            $whereTheseRecords .= " AND ".PRFX."invoice_records.client_id=".$this->db->qstr($client_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count the number of Invoices."));
        } else {
    ...
    public function sum_invoices($value_name, $start_date = null, $end_date = null, $date_type = null, $tax_system = null, $status = null, $employee_id = null, $client_id = null) {
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."invoice_records.client_id=".$this->db->qstr($employee_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not sum the invoice values."));
        } else {
    ...
    public function invoice_build_filter_by_date($start_date = null, $end_date = null, $date_type = null) {
        ...
        if($start_date && $end_date && $date_type) {
            if ($date_type == 'date') {       
                $whereTheseRecords .= " AND ".PRFX."invoice_records.date >= ".$this->db->qstr($start_date)." AND ".PRFX."invoice_records.date <= ".$this->db->qstr($end_date);
            } elseif ($date_type == 'due_date') {       
                $whereTheseRecords .= " AND ".PRFX."invoice_records.due_date >= ".$this->db->qstr($start_date)." AND ".PRFX."invoice_records.due_date <= ".$this->db->qstr($end_date);
            } elseif ($date_type == 'opened_on') {
                $whereTheseRecords .= " AND ".PRFX."invoice_records.opened_on >= ".$this->db->qstr($start_date)." AND ".PRFX."invoice_records.opened_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'closed_on') {       
                $whereTheseRecords .= " AND ".PRFX."invoice_records.closed_on >= ".$this->db->qstr($start_date)." AND ".PRFX."invoice_records.closed_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'last_active') {       
                $whereTheseRecords .= " AND ".PRFX."invoice_records.last_active >= ".$this->db->qstr($start_date)." AND ".PRFX."invoice_records.last_active <= ".$this->db->qstr($end_date.' 23:59:59');
            }
        }
    ...
    public function count_labour_items($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $status = null, $employee_id = null, $client_id = null) {
        ...
        // Filter by Tax System
        if($tax_system) {
            $whereTheseRecords .= " AND ".PRFX."invoice_labour.tax_system=".$this->db->qstr($tax_system);
        }

        // Filter by VAT Tax Code
        if($vat_tax_code) {
            $whereTheseRecords .= " AND ".PRFX."invoice_labour.vat_tax_code=".$this->db->qstr($vat_tax_code);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Failed to count the total number of selected labour items."));
        } else {
    ...
    public function sum_labour_items($value_name, $start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $status = null, $employee_id = null, $client_id = null, $invoice_id = null) {
        ...
        // Filter by Invoice
        if($invoice_id) {
            $whereTheseRecords .= " AND ".PRFX."invoice_labour.invoice_id=".$this->db->qstr($invoice_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Failed to return the sum of labour items selected."));
        } else {
    ...
    public function count_parts_items($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $status = null, $employee_id = null, $client_id = null, $invoice_id = null) {
        ...
        // Filter by Tax System
        if($tax_system) {
            $whereTheseRecords .= " AND ".PRFX."invoice_parts.tax_system=".$this->db->qstr($tax_system);
        }

        // Filter by VAT Tax Code
        if($vat_tax_code) {
            $whereTheseRecords .= " AND ".PRFX."invoice_parts.vat_tax_code=".$this->db->qstr($vat_tax_code);
        }    
        ...
        // Filter by Invoice
        if($invoice_id) {
            $whereTheseRecords .= " AND ".PRFX."invoice_parts.invoice_id=".$this->db->qstr($invoice_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Failed to count the total number of selected parts items."));
        } else {
    ...
    public function count_vouchers($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $type = null, $status = null, $employee_id = null, $client_id = null, $invoice_id = null) {
        ...
        // Filter by Tax System
        if($tax_system) {
            $whereTheseRecords .= " AND ".PRFX."voucher_records.tax_system=".$this->db->qstr($tax_system);
        }

        // Filter by VAT Tax Code
        if($vat_tax_code) {
            $whereTheseRecords .= " AND ".PRFX."voucher_records.vat_tax_code=".$this->db->qstr($vat_tax_code);
        }

        // Filter by Item Type
        if($type) {
            $whereTheseRecords .= " AND ".PRFX."voucher_records.type=".$this->db->qstr($type);
        }
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."voucher_records.employee_id=".$this->db->qstr($employee_id);
        }

        // Filter by Claimed status
        if($client_id && $status != 'claimed') {
            $whereTheseRecords .= " AND ".PRFX."voucher_records.client_id=".$this->db->qstr($client_id);
        }

        // Filter by Invoice
        if($invoice_id) {
            $whereTheseRecords .= " AND ".PRFX."voucher_records.invoice_id=".$this->db->qstr($invoice_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count Vouchers."));
    ...
    public function sum_vouchers($value_name, $start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $type = null, $status = null, $employee_id = null, $client_id = null, $invoice_id = null) {
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."voucher_records.voucher_records.employee_id=".$this->db->qstr($employee_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Failed to return the sum value for the selected Vouchers."));
        } else {
    ...
    public function voucher_build_filter_by_status($status = null, $client_id = null) {
        ...
        if($status) {
            ...
            if($status == 'open') {            
                ...
                $whereTheseRecords .= " AND ".PRFX."voucher_records.redeemed_client_id = ".$this->db->qstr($client_id);
            } else {
                $whereTheseRecords .= " AND ".PRFX."voucher_records.status = ".$this->db->qstr($status);                       
            }
    ...
    public function voucher_build_filter_by_date($start_date = null, $end_date = null, $date_type = null) {
        ...
        if($start_date && $end_date) {
            ...
            if($date_type == 'date') {       
                ...
                $whereTheseRecords .= " AND ".PRFX."voucher_records.opened_on >= ".$this->db->qstr($start_date)." AND ".PRFX."voucher_records.opened_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type== 'expiry') {
                $whereTheseRecords .= " AND ".PRFX."voucher_records.expiry_date >= ".$this->db->qstr($start_date)." AND ".PRFX."voucher_records.expiry_date <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'redeemed_on') {
                $whereTheseRecords .= " AND ".PRFX."voucher_records.redeemed_on >= ".$this->db->qstr($start_date)." AND ".PRFX."voucher_records.redeemed_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'closed_on') {
                $whereTheseRecords .= " AND ".PRFX."voucher_records.closed_on >= ".$this->db->qstr($start_date)." AND ".PRFX."voucher_records.closed_on <= ".$this->db->qstr($end_date.' 23:59:59');
            }
    ...
    public function count_refunds($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $item_type = null, $status = null, $employee_id = null, $client_id = null) {
        ...
        // Filter by Tax System
        if($tax_system) {
            $whereTheseRecords .= " AND ".PRFX."refund_records.tax_system=".$this->db->qstr($tax_system);
        }

        // Filter by VAT Tax Code
        if($vat_tax_code) {
            $whereTheseRecords .= " AND ".PRFX."refund_records.vat_tax_code=".$this->db->qstr($vat_tax_code);
        }

        // Filter by Item Type
        if($item_type) {
            $whereTheseRecords .= " AND ".PRFX."refund_records.item_type=".$this->db->qstr($item_type);
        }
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."refund_records.employee_id=".$this->db->qstr($employee_id);
        }

        // Filter by Client
        if($client_id) {
            $whereTheseRecords .= " AND ".PRFX."refund_records.client_id=".$this->db->qstr($client_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count Refunds."));
    ...
    public function sum_refunds($value_name, $start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $item_type = null, $status = null, $employee_id = null, $client_id = null) {
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Failed to return the sum value for the selected Refunds."));
        } else {
    ...
    public function refund_build_filter_by_date($start_date = null, $end_date = null, $date_type = null) {
        ...
        if($start_date && $end_date && $date_type) {
            if ($date_type == 'date') {       
                $whereTheseRecords .= " AND ".PRFX."refund_records.date >= ".$this->db->qstr($start_date)." AND ".PRFX."refund_records.date <= ".$this->db->qstr($end_date);
            } elseif ($date_type == 'opened_on') {
                $whereTheseRecords .= " AND ".PRFX."refund_records.opened_on >= ".$this->db->qstr($start_date)." AND ".PRFX."refund_records.opened_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'closed_on') {       
                $whereTheseRecords .= " AND ".PRFX."refund_records.closed_on >= ".$this->db->qstr($start_date)." AND ".PRFX."refund_records.closed_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'last_active') {       
                $whereTheseRecords .= " AND ".PRFX."refund_records.last_active >= ".$this->db->qstr($start_date)." AND ".PRFX."refund_records.last_active <= ".$this->db->qstr($end_date.' 23:59:59');
            }
        }
    ...
    public function refund_build_filter_by_status($status = null) {
        ...
        if($status) {   
            if($status == 'open') {            
                ...
                $whereTheseRecords .= " AND ".PRFX."refund_records.status= ".$this->db->qstr($status);            
            }
        }
    ...
    public function count_expenses($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $item_type = null, $status = null, $employee_id = null, $invoice_id = null) {
        ...
        // Filter by Tax System
        if($tax_system) {
            $whereTheseRecords .= " AND ".PRFX."expense_records.tax_system=".$this->db->qstr($tax_system);
        }

        // Filter by VAT Tax Code
        if($vat_tax_code) {
            $whereTheseRecords .= " AND ".PRFX."expense_records.vat_tax_code=".$this->db->qstr($vat_tax_code);
        }    

        // Filter by Item Type
        if($item_type) {
            $whereTheseRecords .= " AND ".PRFX."expense_records.item_type=".$this->db->qstr($item_type);
        }
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."expense_records.employee_id=".$this->db->qstr($employee_id);
        }

        // Filter by invoice_id
        if($invoice_id) {
            $whereTheseRecords .= " AND ".PRFX."expense_records.invoice_id=".$this->db->qstr($invoice_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count Expenses."));
    ...
    public function sum_expenses($value_name, $start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $item_type = null, $status = null, $employee_id = null) {
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Failed to return the sum value for the selected expenses."));
        } else {
    ...
    public function expense_build_filter_by_date($start_date = null, $end_date = null, $date_type = null) {
        ...
        if($start_date && $end_date && $date_type) {
            ...
            if ($date_type == 'date') {       
                $whereTheseRecords .= " AND ".PRFX."expense_records.date >= ".$this->db->qstr($start_date)." AND ".PRFX."expense_records.date <= ".$this->db->qstr($end_date);
            } elseif ($date_type == 'opened_on') {
                $whereTheseRecords .= " AND ".PRFX."expense_records.opened_on >= ".$this->db->qstr($start_date)." AND ".PRFX."expense_records.opened_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'closed_on') {       
                $whereTheseRecords .= " AND ".PRFX."expense_records.closed_on >= ".$this->db->qstr($start_date)." AND ".PRFX."expense_records.closed_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'last_active') {       
                $whereTheseRecords .= " AND ".PRFX."expense_records.last_active >= ".$this->db->qstr($start_date)." AND ".PRFX."expense_records.last_active <= ".$this->db->qstr($end_date.' 23:59:59');
            }
        }
    ...
    public function expense_build_filter_by_status($status = null) {
        ...
        if($status) {   
            if($status == 'open') {            
                ...
                $whereTheseRecords .= " AND ".PRFX."expense_records.status= ".$this->db->qstr($status);            
            }
        }
    ...
    public function count_otherincomes($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $item_type = null, $status = null, $employee_id = null) {
        ...
        // Filter by Tax System
        if($tax_system) {
            $whereTheseRecords .= " AND ".PRFX."otherincome_records.tax_system=".$this->db->qstr($tax_system);
        }

        // Filter by VAT Tax Code
        if($vat_tax_code) {
            $whereTheseRecords .= " AND ".PRFX."otherincome_records.vat_tax_code=".$this->db->qstr($vat_tax_code);
        }

        // Filter by Item Type
        if($item_type) {
            $whereTheseRecords .= " AND ".PRFX."otherincome_records.item_type=".$this->db->qstr($item_type);
        }
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."otherincome_records.employee_id=".$this->db->qstr($employee_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count other incomes."));
    ...
    public function sum_otherincomes($value_name, $start_date = null, $end_date = null, $date_type = null, $tax_system = null, $vat_tax_code = null, $item_type = null, $status = null, $employee_id = null) {
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Failed to return the sum value for the selected other incomes."));
        } else {
    ...
    public function otherincome_build_filter_by_date($start_date = null, $end_date = null, $date_type = null) {
        ...
        if($start_date && $end_date && $date_type) {
            if ($date_type == 'date') {       
                $whereTheseRecords .= " AND ".PRFX."otherincome_records.date >= ".$this->db->qstr($start_date)." AND ".PRFX."otherincome_records.date <= ".$this->db->qstr($end_date);
            } elseif ($date_type == 'opened_on') {
                $whereTheseRecords .= " AND ".PRFX."otherincome_records.opened_on >= ".$this->db->qstr($start_date)." AND ".PRFX."otherincome_records.opened_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'closed_on') {       
                $whereTheseRecords .= " AND ".PRFX."otherincome_records.closed_on >= ".$this->db->qstr($start_date)." AND ".PRFX."otherincome_records.closed_on <= ".$this->db->qstr($end_date.' 23:59:59');
            } elseif ($date_type == 'last_active') {       
                $whereTheseRecords .= " AND ".PRFX."otherincome_records.last_active >= ".$this->db->qstr($start_date)." AND ".PRFX."otherincome_records.last_active <= ".$this->db->qstr($end_date.' 23:59:59');
            }
        }
    ...
    public function otherincome_build_filter_by_status($status = null) {
        ...
        if($status) {   
            if($status == 'open') {            
                ...
                $whereTheseRecords .= " AND ".PRFX."otherincome_records.status= ".$this->db->qstr($status);            
            }
        }
    ...
    public function count_payments($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $status = null, $type = null, $method = null, $employee_id = null, $client_id = null, $invoice_id = null, $refund_id = null, $expense_id = null, $otherincome_id = null) {   
        ...
        // Filter by Tax System
        if($tax_system) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.tax_system=".$this->db->qstr($tax_system);
        }

        // Restrict by Status
        if($status) {   
            $whereTheseRecords .= " AND ".PRFX."payment_records.status= ".$this->db->qstr($status);  
        }
        ...
        // Filter by Method
        if($method) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.method=".$this->db->qstr($method);
        }

        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.employee_id=".$this->db->qstr($employee_id);
        }

        // Filter by Client
        if($client_id) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.client_id=".$this->db->qstr($client_id);
        }

        // Filter by Invoice
        if($invoice_id) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.invoice_id=".$this->db->qstr($invoice_id);
        }

        // Filter by Refund
        if($refund_id) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.refund_id=".$this->db->qstr($refund_id);
        }

        // Filter by Expense
        if($expense_id) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.expense_id=".$this->db->qstr($expense_id);
        }

        // Filter by Otherincome
        if($otherincome_id) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.otherincome_id=".$this->db->qstr($otherincome_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count the number of payments."));
        } else {
    ...
    public function sum_payments($start_date = null, $end_date = null, $date_type = null, $tax_system = null, $status = null, $type = null, $method = null, $employee_id = null, $client_id = null, $invoice_id = null, $refund_id = null, $expense_id = null, $otherincome_id = null) {
        ...
        // Filter by Employee
        if($employee_id) {
            $whereTheseRecords .= " AND ".PRFX."payment_records.client_id=".$this->db->qstr($employee_id);
        }
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not sum the payment values."));
        } else {
    ...
    public function payment_build_filter_by_date($start_date = null, $end_date = null, $date_type = null) {
        ...
        if($start_date && $end_date && $date_type) {
            if ($date_type == 'date') {       
                $whereTheseRecords .= " AND ".PRFX."payment_records.date >= ".$this->db->qstr($start_date)." AND ".PRFX."payment_records.date <= ".$this->db->qstr($end_date);
            } elseif ($date_type == 'last_active') {       
                $whereTheseRecords .= " AND ".PRFX."payment_records.last_active >= ".$this->db->qstr($start_date)." AND ".PRFX."payment_records.last_active <= ".$this->db->qstr($end_date.' 23:59:59');
            }
        }
    ...
    public function revenue_payment_prorated_against_record($payment_id, $record_type) {
        ...
        if($record_type == 'refund') {$record_details = $this->app->components->refund->$this->app->components->refund->get_refund_details($payment_details['refund_id']);}
    ...
    public function count_suppliers() { 
        ...
        if(!$rs = $this->db->Execute($sql)) {
            $this->app->system->general->force_error_page('database', __FILE__, __FUNCTION__, $this->db->ErrorMsg(), $sql, _gettext("Could not count the number of suppliers."));
        } else {
