...
class Client {
    ...
    function insert_client_note($client_id, $note) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            // Log activity        
            $record = _gettext("A new client note was added to the client").' '.get_client_details($client_id, 'display_name').' '._gettext("by").' '.\Factory::getUser()->login_display_name.'.';
            write_record_to_activity_log($record, \Factory::getUser()->login_user_id, $client_id);
    ...
    function update_client($qform) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            // Log activity        
            $record = _gettext("The client").' '.get_client_details($qform['client_id'], 'display_name').' '._gettext("was updated by").' '.\Factory::getUser()->login_display_name.'.';
    ...
    function update_client_note($client_note_id, $note) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            // Log activity        
            $record = _gettext("Client Note").' '.$client_note_id.' '._gettext("for").' '.get_client_details($client_id, 'display_name').' '._gettext("was updated by").' '.\Factory::getUser()->login_display_name.'.';
            write_record_to_activity_log($record, \Factory::getUser()->login_user_id, $client_id);
    ...
    function delete_client($client_id) {
        ...
        $record = _gettext("The client").' '.$client_details['display_name'].' '._gettext("has been deleted by").' '.\Factory::getUser()->login_display_name.'.';
    ...
    function delete_client_note($client_note_id) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            // Log activity        
            $record = _gettext("Client Note").' '.$client_note_id.' '._gettext("for Client").' '.$client_details['display_name'].' '._gettext("was deleted by").' '.\Factory::getUser()->login_display_name.'.';
    ...
    function build_googlemap_directions_string($client_id, $employee_id) {
        ...
        $google_server = rtrim(\Factory::getConfig()->get('google_server', 'https://www.google.com/'), '/');
