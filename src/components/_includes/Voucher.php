...
class Voucher {
    ...
    function insert_voucher($invoice_id, $type, $expiry_date, $unit_net, $note) {
        ...
        if(!$db->execute($sql)) {
            ...
            // Log activity        
            $record = _gettext("Voucher").' '.$voucher_id.' '._gettext("was created by").' '.\Factory::getUser()->login_display_name.'.';      
            write_record_to_activity_log($record, \Factory::getUser()->login_user_id, $invoice_details['client_id']);
    ...
    function update_voucher($voucher_id, $expiry_date, $unit_net, $note) {
        ...
        if(!$db->execute($sql)) {
            ...
            // Log activity
            $record = _gettext("Voucher").' '.$voucher_id.' '._gettext("was updated by").' '.\Factory::getUser()->login_display_name.'.';
    ...
    function update_voucher_status($voucher_id, $new_status, $silent = false) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            insert_workorder_history_note($voucher_details['workorder_id'], _gettext("Voucher Status updated to").' '.$voucher_status_display_name.' '._gettext("by").' '.\Factory::getUser()->login_display_name.'.');

            // Log activity        
            $record = _gettext("Voucher").' '.$voucher_id.' '._gettext("Status updated to").' '.$voucher_status_display_name.' '._gettext("by").' '.\Factory::getUser()->login_display_name.'.';
    ...
    function update_voucher_as_redeemed($voucher_id, $invoice_id, $payment_id) {
        ...
        if(!$rs = $db->execute($sql)) {
            ...
            write_record_to_activity_log($record, \Factory::getUser()->login_user_id, $voucher_details['client_id'], null, $invoice_id);
    ...
    function refund_voucher($voucher_id, $refund_id) {
        ...
        // Create a Workorder History Note  
        insert_workorder_history_note($voucher_details['voucher_id'], _gettext("Voucher").' '.$voucher_id.' '._gettext("was refunded by").' '.\Factory::getUser()->login_display_name.'.');

        // Log activity        
        $record = _gettext("Voucher").' '.$voucher_id.' '._gettext("for Invoice").' '.$voucher_details['invoice_id'].' '._gettext("was refunded by").' '.\Factory::getUser()->login_display_name.'.';
        write_record_to_activity_log($record, $voucher_details['employee_id'], $voucher_details['client_id'], $voucher_details['workorder_id'], $voucher_id);
    ...
    function revert_refund_voucher($voucher_id) {
        ...
        // Create a Workorder History Note  
        insert_workorder_history_note($voucher_details['voucher_id'], _gettext("Voucher").' '.$voucher_id.' '._gettext("was refund was reverted by").' '.\Factory::getUser()->login_display_name.'.');

        // Log activity        
        $record = _gettext("Voucher").' '.$voucher_id.' '._gettext("for Invoice").' '.$voucher_details['invoice_id'].' '._gettext("refund was reverted by").' '.\Factory::getUser()->login_display_name.'.';
        write_record_to_activity_log($record, $voucher_details['employee_id'], $voucher_details['client_id'], $voucher_details['workorder_id'], $voucher_id);
    ...
    function cancel_voucher($voucher_id) {     
        ...
        if(!check_single_voucher_can_be_cancelled($voucher_id)) {
            ...
            // Log activity        
            $record = _gettext("Voucher").' '.$voucher_id.' '._gettext("was cancelled by").' '.\Factory::getUser()->login_display_name.'.';
            write_record_to_activity_log($record, $voucher_details['employee_id'], $voucher_details['client_id']);
    ...
    function delete_voucher($voucher_id) {     
        ...
        if(!check_single_voucher_can_be_deleted($voucher_id)) {
            ...
            if(!$db->execute($sql)) {
                ...
                // Log activity        
                $record = _gettext("Voucher").' '.$voucher_id.' '._gettext("was deleted by").' '.\Factory::getUser()->login_display_name.'.';
                write_record_to_activity_log($record, $voucher_details['employee_id'], $voucher_details['client_id']);
