...
class Expense {
    ...
    function insert_expense($qform) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            insert_workorder_history_note($invoice_details['workorder_id'], _gettext("Expense").' '.$db->Insert_ID().' '._gettext("added").' '._gettext("by").' '.\Factory::getUser()->login_display_name.'.');

            // Log activity        
            $record = _gettext("Expense Record").' '.$db->Insert_ID().' '._gettext("created.");
            write_record_to_activity_log($record, \Factory::getUser()->login_user_id, $invoice_details['workorder_id'], $invoice_details['client_id'], $qform['invoice_id']);
    ...
    function update_expense($qform) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            // Create a Workorder History Note
            insert_workorder_history_note($invoice_details['workorder_id'], _gettext("Expense").' '.$expense_id.' '._gettext("updated").' '._gettext("by").' '.\Factory::getUser()->login_display_name.'.');
    ...
    function update_expense_status($expense_id, $new_status, $silent = false) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            insert_workorder_history_note($invoice_details['workorder_id'], _gettext("Expense Status updated to").' '.$expense_status_display_name.' '._gettext("by").' '.\Factory::getUser()->login_display_name.'.');

            // Log activity        
            $record = _gettext("Expense").' '.$expense_id.' '._gettext("Status updated to").' '.$expense_status_display_name.' '._gettext("by").' '.\Factory::getUser()->login_display_name.'.';
            write_record_to_activity_log($record, \Factory::getUser()->login_user_id, $invoice_details['client_id'], $invoice_details['workorder_id'], $expense_details['invoice_id']);
    ...
    function cancel_expense($expense_id) {
        ...
        /*Create a Workorder History Note  
        insert_workorder_history_note($invoice_details['workorder_id'], _gettext("Expense").' '.$expense_id.' '._gettext("was cancelled by").' '.\Factory::getUser()->login_display_name.'.');*/

        // Log activity        
        $record = _gettext("Expense").' '.$expense_id.' '._gettext("was cancelled by").' '.\Factory::getUser()->login_display_name.'.';
    ...
    function delete_expense($expense_id) {
        ...
        if(!$rs = $db->Execute($sql)) {
            ...
            /* Create a Workorder History Note  
            insert_workorder_history_note($invoice_details['workorder_id'], _gettext("Expense").' '.$expense_id.' '._gettext("was deleted by").' '.\Factory::getUser()->login_display_name.'.');*/
            ...
            /* Log activity        
            $record = _gettext("Expense Record").' '.$expense_id.' '._gettext("deleted.");
            write_record_to_activity_log($record, \Factory::getUser()->login_user_id, $invoice_details['client_id'], $invoice_details['workorder_id'], $invoice_id);
