...
$barcode = $bc_generator->getBarcode(get_voucher_details(\QFactory::$VAR['voucher_id'], 'voucher_code'), $bc_generator::TYPE_CODE_128);
...
if(\QFactory::$VAR['print_content'] == 'voucher') {    
    
    // Build the PDF filename
    $pdf_filename = _gettext("Voucher").' '.\QFactory::$VAR['voucher_id'];
    ...
    // Print PDF
        ...
        // Log activity
        $record = _gettext("Voucher").' '.\QFactory::$VAR['voucher_id'].' '._gettext("has been printed as a PDF.");
        write_record_to_activity_log($record, $voucher_details['employee_id'], $voucher_details['client_id'], $voucher_details['workorder_id'], $voucher_details['invoice_id']);
    ...
    // Email PDF
        ...
        // Log activity
        $record = _gettext("Voucher").' '.\QFactory::$VAR['voucher_id'].' '._gettext("has been emailed as a PDF.");
        write_record_to_activity_log($record, $voucher_details['employee_id'], $voucher_details['client_id'], $voucher_details['workorder_id'], $voucher_details['invoice_id']);
        
        // Email the PDF
        send_email($client_details['email'], _gettext("Voucher").' '.\QFactory::$VAR['voucher_id'], $body, $client_details['display_name'], $attachment);
