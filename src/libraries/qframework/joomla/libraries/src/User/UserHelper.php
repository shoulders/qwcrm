...
abstract class UserHelper
{
	 ...
	 */
	public static function addUserToGroup($user_id, $groupId)
	{
		// Get the user object.
		...
		$temp         = User::getInstance((int) $user_id);
	 ...
	 * @since   1.7.0
	 */
	public static function removeUserFromGroup($user_id, $groupId)
	{
		// Get the user object.
		$user = User::getInstance((int) $user_id);
		...
		// Set the group data for any preloaded user objects.
		$temp = \JFactory::getUser((int) $user_id);
		$temp->groups = $user->groups;
	...
	public static function setUserGroups($user_id, $groups)
	{
		...
		{
			// Set the group data for any preloaded user objects.
			$temp = \Factory::getUser((int) $user_id);
			$temp->groups = $user->groups;
	 ...
	 * @since   1.7.0
	 */
	public static function getProfile($user_id = 0)
	{
		if ($user_id == 0)
		{
	...
	{
		// This is deprecated in Joomla 4.0 and is poitnless anyway
	 ...
	 *
	 * @param   string  $user_id      User ID for this user
	 ...
	 */
	public static function invalidateCookie($user_id, $cookieName)
	{
		$db = \JFactory::getDbo();
		$query = $db->getQuery(true);
		...
		// Destroy the cookie in the browser. (my version if this function were to be used)
		//$cookie = new QCookie;
		//$cookie->set($cookieName, false, time() - 42000, $this->config->get('cookie_path', '/'), $this->config->get('cookie_domain'));  
	 ...
	 * @since   3.6.5
	 */
	public static function checkSuperUserInUsers(array $user_ids)
	{
		foreach ($user_ids as $user_id)
		{
			foreach (static::getUserGroups($user_id) as $userGroupId)
			{
	 ...
	 * @since   3.9.28
	 */
	public static function destroyUserSessions($user_id, $keepCurrent = false, $clientId = null)
	{
		$db = \JFactory::getDbo();
		...
		$query = $db->getQuery(true)
			...
			->where($db->quoteName('userid') . ' = ' . (int) $user_id);
