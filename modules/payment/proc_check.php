...
if(empty($cheque_amount)) {
    force_page("payment", "new&error_msg=Please Fill in the check amount.&workorder_id=$workorder_id&customer_id=$customer_id&invoice_id=$invoice_id&page_title=Billing");
}

if(empty($cheque_recieved)) {
    force_page("payment", "new&error_msg=Please Fill in the Document Number.&workorder_id=$workorder_id&customer_id=$customer_id&invoice_id=$invoice_id&page_title=Billing");
}
...
if($invoice_details['INVOICE_AMOUNT'] > $cheque_amount){
    ...
    /* full payment made */
    if($invoice_details['INVOICE_AMOUNT'] < $cheque_amount) {
        force_page('payment', 'new&workorder_id='.$workorder_id.'&customer_id='.$customer_id.'    &invoice_id='.$invoice_id.'&error_msg=You can not bill more than the amout of the invoice.');
            exit;
        } 
    if($invoice_details['INVOICE_AMOUNT'] == $cheque_amount){    
        /* insert Transaction */
