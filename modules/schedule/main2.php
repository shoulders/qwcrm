...
$H = get_setup_info($db, 'OPENING_HOUR');
$E = get_setup_info($db, 'CLOSING_HOUR');
if(empty($H) || empty($E)) {
...
$employees =  display_employees_info($db);
...
for ($kk=0; $kk<7; $kk++) {
    ...
    if (in_array($kk, $daylist)) {
        ...
        } elseif ($kk == $daylist[count($daylist)-1]) {
