...
if(!isset($VAR['customer_id']) || $VAR['customer_id'] === '' ) {   
    force_page('core', 'error', 'error_type=warning&error_location=workorder:new&php_function=&error_msg='.$smarty->get_template_vars('translate_workorder_error_message_new_loadpage_failed').'&php_errormsg='.$php_errormsg.'&database_error='.$db->ErrorMsg());
    exit;
} else {
