...
if(isset($submit)){
    ...
    if($VAR['labour_hour'] > 0 ) {
        ...
        foreach($VAR['labour_hour'] as $key=>$val) {
            ...
            $sql .="(".
                    ...
                    $db->qstr($val).",".
                    $db->qstr($val * $VAR['labour_rate'][$i]).
                    
                    "),";            
            
            $temp_sub_total = $temp_sub_total + ($val * $VAR['rate'][$i]);
    ...
    if($VAR['parts_qty'] > 0 ) {
        ...
        foreach($VAR['parts_qty'] as $key=>$val) {
            $sql .="(".
                $db->qstr($VAR['invoice_id']).",".
                $db->qstr($VAR['manufacture'][$i]).",'',".
                $db->qstr($VAR['parts_description'][$i]).",'',".
                $db->qstr($VAR['parts_price'][$i]).",".
                $db->qstr($val).", ".
                $db->qstr($val * $VAR['parts_price'][$i])."),";
            
            $temp_sub_total = $temp_sub_total + ($val * $VAR['parts_price'][$i]);
            $i++;
        }
        $sql = substr($sql ,0,-1);
        if(!$rs = $db->Execute($sql)) {
...
if(empty($VAR['discount'])) {
    $sql = "SELECT DISCOUNT FROM ".PRFX."TABLE_CUSTOMER WHERE CUSTOMER_ID =$customer_id";
    $rs = $db->execute($sql);
...
// Calculate Tax
$sql = "SELECT INVOICE_TAX FROM ".PRFX."TABLE_COMPANY";
$rs = $db->execute($sql);
...
if (!isset ($paid_amount)) {
    $sql = "SELECT PAID_AMOUNT FROM ".PRFX."TABLE_INVOICE WHERE INVOICE_ID =".$VAR['invoice_id'];
    $rs = $db->execute($sql);
...
############################################
    ...
    $sql = "SELECT count(*) as count FROM ".PRFX."TABLE_INVOICE WHERE WORKORDER_ID=".$db->qstr($workorder_id);
    $rs = $db->Execute($sql);
