...
/* html Print out */
	...
	$smarty->assign('pay_pal', $pay_pal);
	$smarty->assign('company',$company);
	$smarty->assign('company2',$company2);
	//$smarty->assign('ppamount',$ppamount);
...
$ctankyou = $company1['COMPANY_THANKYOU'];
...
{
    $this->SetFont( "Helvetica", "", 10);
    $length = $this->GetStringWidth( "Comments : " . $ctankyou );
    $r1  = 10;
    $r2  = $r1 + $length;
    $y1  = $this->h - 45.5;
    $y2  = $y1+5;
    $this->SetXY( $r1 , $y1 );
    $this->Cell($length,4, "Comments : " . $ctankyou);
}
...
$pdf->addCompany( "$cname",
                  ...
                  "E: $cemail\n" .
                  "ABN: $cabn\n");
$pdf->fact_dev("INVOICE # ",$invoice[INVOICE_ID]);
...
$pdf->addClientAddress("$cusnamef $cusnamel\n" .
                        "$cusaddress\n" .
...
$pdf->AddRemark($ctankyou);
$pdf->Output("cache/invoice.pdf", "F");

// email stuff (change data below)
$to = "newuser@localhost"; 
$from = "newuser@localhost"; 
$subject = "send email with pdf attachment"; 
//$message = "<a>Please see the attachment.</a>";
