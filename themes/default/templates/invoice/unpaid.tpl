                                    ...
                                    <table class="menutable" width="100%" border="0" cellpadding="5" cellspacing="0">
                                        <tr>
                                            ...
                                            <td valign="top" nowrap align="right">
                                                <form id="1">
                                                    <a href="?page=invoice:unpaid&name={$name}&submit=submit&page_no=1"><img src="{$theme_images_dir}rewnd_24.gif" border="0"></a>&nbsp;
                                                    {if $previous != ''}
                                                        <a href="?page=invoice:unpaid&name={$name}&submit=submit&page_no={$previous}"><img src="{$theme_images_dir}back_24.gif" border="0"></a>&nbsp;
                                                    {/if}
                                                    <select id="changeThisPage" onChange="changePage();">
                                                        {section name=page loop=$total_pages start=1}
                                                            <option value="?page=invoice:unpaid&name={$name}&submit=submit&page_no={$smarty.section.page.index}" {if $page_no == $smarty.section.page.index } Selected {/if}>{$translate_invoice_page}  {$smarty.section.page.index} {$translate_invoice_of} {$total_pages} </option>
                                                        {/section}
                                                        <option value="?page=invoice:unpaid&name={$name}&submit=submit&page_no={$total_pages}" {if $page_no == $total_pages} selected {/if}>{$translate_invoice_page}  {$total_pages} {$translate_invoice_of} {$total_pages}</option>
                                                    </select>
                                                    {if $next != ''}
                                                        <a href="?page=invoice:unpaid&name={$name}&submit=submit&page_no={$next}"><img src="{$theme_images_dir}forwd_24.gif" border="0"></a>
                                                    {/if}
                                                    <a href="?page=invoice:unpaid&name={$name}&submit=submit&page_no={$total_pages}"><img src="{$theme_images_dir}fastf_24.gif" border="0"></a>
                                                    <br>
                                            ...
                                            <td valign="top" colspan="2">
                                                <table class="olotable" width="100%" border="0" cellpadding="5" cellspacing="0">
                                                    ...
                                                    {section name=q loop=$invoice}
                                                        <tr onmouseover="this.className='row2';" onmouseout="this.className='row1';" onDblClick="window.location='index.php?page=invoice:edit&invoice_id={$invoice[q].INVOICE_ID}';" class="row1">
                                                            <td class="olotd4" nowrap><a href="index.php?page=invoice:edit&invoice_id={$invoice[q].INVOICE_ID}">{$invoice[q].INVOICE_ID}</a></td>
                                                            ...
                                                            <td class="olotd4" nowrap><a href="index.php?page=workorder:details&workorder_id={$invoice[q].WORKORDER_ID}&page_title=Work%20Order%20ID%20{$invoice[q].WORKORDER_ID}">{$invoice[q].WORKORDER_ID}</a></td>
                                                            <td class="olotd4" nowrap><img src="{$theme_images_dir}icons/16x16/view.gif" border="0" onMouseOver="ddrivetip('<center><b>Contact</b></center><hr><b>Work: </b>{$invoice[q].EMPLOYEE_WORK_PHONE}<br><b>Mobile: </b>{$invoice[q].EMPLOYEE_MOBILE_PHONE}<br><b>Home: </b>{$invoice[q].EMPLOYEE_HOME_PHONE}');" onMouseOut="hideddrivetip();"><a  href="?page=employee:details&employee_id={$invoice[q].EMPLOYEE_ID}&page_title={$invoice[q].EMPLOYEE_DISPLAY_NAME}"> {$invoice[q].EMPLOYEE_DISPLAY_NAME}</td>
