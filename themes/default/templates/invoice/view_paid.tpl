                                            ...
                                            <td valign="top" nowrap align="right">
                                                <form id="1" action="">
                                                    ...
                                                    <select name="page_no" onChange="changePage();">
                                                        ...
                                                        {/section}
                                                        <option value="?page=invoice:view_paid&submit=submit&page_no={$total_pages}" {if $page_no == $total_pages} selected {/if}>{$translate_invoice_page}  {$total_pages} {$translate_invoice_of} {$total_pages}</option>
                                                    </select>
                                                    {if $next != ''}
                                                ...
                                                <table class="olotable" width="100%" border="0" cellpadding="5" cellspacing="0">
                                                    ...
                                                    {section name=q loop=$invoice}
                                                        <tr onmouseover="this.className='row2';" onmouseout="this.className='row1';" onDblClick="window.location='index.php?page=invoice:view&invoice_id={$invoice[q].INVOICE_ID}&page_title={$translate_invoice_invoice}&customer_id={$invoice[q].CUSTOMER_ID}';" class="row1">
                                                            <td class="olotd4" nowrap><a href="index.php?page=invoice:view&invoice_id={$invoice[q].INVOICE_ID}&workorder_id={$workorder_id}&page_title=Invoice&customer_id={$invoice[q].CUSTOMER_ID}">{$invoice[q].INVOICE_ID}</a></td>
                                                            ...
                                                            <td class="olotd4" nowrap><a href="index.php?page=workorder:details&workorder_id={$invoice[q].WORKORDER_ID}&workorder_id={$workorder_id}&page_title={$translate_invoice_workorder_id}{$invoice[q].WORKORDER_ID}">{$invoice[q].WORKORDER_ID}</a></td>
                                                            <td class="olotd4" nowrap><img src="{$theme_images_dir}icons/16x16/view.gif" alt="" border="0" onMouseOver="ddrivetip('<b>Work: </b>{$invoice[q].EMPLOYEE_WORK_PHONE}<br><b>Mobile: </b>{$invoice[q].EMPLOYEE_MOBILE_PHONE}<br><b>Home: </b>{$invoice[q].EMPLOYEE_HOME_PHONE}');" onMouseOut="hideddrivetip();"><a  href="?page=employee:employee_details&employee_id={$invoice[q].EMPLOYEE_ID}&page_title={$invoice[q].EMPLOYEE_DISPLAY_NAME}">{$invoice[q].EMPLOYEE_DISPLAY_NAME}</a></td>
