<!-- attribution.tpl -->
<p>This page will show the used software, their version and where i got them from. This will should cover any attributions that are needed or a particular license</p>
<p>This also assumes you are usign the default template.</p>
<ul>
    <li>Name and version | License | URL | What it does?</li>
    <li>TinyMCE - Version 4.5.0 (2016-11-23) | LGPL v2.1 (or later)</li>
    <li>dhtmlxCombo v50 Standardd| GPL v2 (or later)</li>
    <li>jquery v3.1.1 | jquery.org/license</li>
    <li>swiftmailer v5.4.4 | MIT | https://github.com/swiftmailer/swiftmailer</li>
    <li>FPDF v1.81 (2015-12-20) | permissive | http://www.fpdf.org/</li>
    <li>Slashdot Menu (SDMenu) (2007-03-22) | no license specified, but seems free | http://dynamicdrive.com/dynamicindex1/slashdot.htm</li>
    <li>Cool DHMTL Tooltip (04-12-16) | Dynamic Drive License http://www.dynamicdrive.com/notice.htm | http://www.dynamicdrive.com/dynamicindex5/dhtmltooltip.htm </li>
    <li>JSCal2 v1.9 (2011/03/13) | permissive? | http://www.dynarch.com/jscal/</li>
    <li>TCPDF v6.2.13 | LGPL v3 | http://www.tcpdf.org - no longer used?</li>
    <li>mPDF v6.1.0 | GPLv2 | https://github.com/mpdf/mpdf</li>
    <li>PHPtoICS.php iCalendar script | free to use? | https://gist.github.com/jakebellacera/635416 - i did not use this</li>
    <li>ADODB V4.66</li>
    <li>Smarty v2.6.9</li>
</ul>