...
<table class="olotable" width="100%" border="0" cellpadding="4" cellspacing="0">
    ...
    {foreach from=$payment item=payment}
        {if $payment.WORK_ORDER_ID > 0 }
            <tr onmouseover="this.className='row2';" onmouseout="this.className='row1';" onDblClick="window.location='?page=workorder:details&wo_id={$payment.WORK_ORDER_ID}&customer_id={$payment.CUSTOMER_ID}&page_title={$translate_workorder_work_order_id}  {$payment.WORK_ORDER_ID}';" class="row1">
                <td class="olotd4"><a href="?page=workorder:details&wo_id={$payment.WORK_ORDER_ID}&customer_id={$payment.CUSTOMER_ID}&page_title={$translate_workorder_work_order_id}  {$payment.WORK_ORDER_ID}">{$payment.WORK_ORDER_ID}<a/></td>
                ...
                {else}
                <img src="{$theme_images_dir}icons/16x16/view.gif" border="0"
                     onMouseOver="ddrivetip('<center><b>{$translate_workorder_contact_info}</b></center><hr><b>{$translate_workorder_fax}: </b>{$payment.EMPLOYEE_WORK_PHONE}<br><b>{$translate_workorder_mobile}: </b>{$payment.EMPLOYEE_MOBILE_PHONE}<br><b>{$translate_workorder_home}: </b>{$payment.EMPLOYEE_HOME_PHONE}');"
                     onMouseOut="hideddrivetip();"> 
                <a class="link1" href="?page=employee:employee_details&employee_id={$payment.EMPLOYEE_ID}&page_title={$payment.EMPLOYEE_DISPLAY_NAME}">{$payment.EMPLOYEE_DISPLAY_NAME}</a>
                {/if}
                </td>
                <td class="olotd4" align="center" nowrap>
                    <a href="?page=workorder:print&wo_id={$payment.WORK_ORDER_ID}&customer_id={$payment.CUSTOMER_ID}&page_title={$translate_workorder_print_work_order_id} {$payment.WORK_ORDER_ID}&theme=off">
                    ...
                    <a href="?page=workorder:details&wo_id={$payment.WORK_ORDER_ID}&customer_id={$payment.CUSTOMER_ID}&page_title={$translate_workorder_work_order_id} {$payment.WORK_ORDER_ID}">
