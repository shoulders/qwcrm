...
<table class="olotable" width="100%" border="0" cellpadding="4" cellspacing="0">
    ...
    {section name=u loop=$unpaid_workorders}
        <tr onmouseover="this.className='row2';" onmouseout="this.className='row1';" onDblClick="window.location='index.php?page=workorder:details&workorder_id={$unpaid_workorders[u].workorder_id}&customer_id={$unpaid_workorders[u].customer_id}';" class="row1">
            ...
            <!-- ID -->
            <td class="olotd4"><a href="index.php?page=workorder:details&workorder_id={$unpaid_workorders[u].workorder_id}&customer_id={$unpaid_workorders[u].customer_id}">{$unpaid_workorders[u].workorder_id}<a/></td>

            <!-- Opened -->
            <td class="olotd4"> {$unpaid_workorders[u].open_date|date_format:$date_format}</td>
            ...
            <td class="olotd4" align="center">
                {if $unpaid_workorders[u].status == '1'}{t}WORKORDER_STATUS_1{/t}{/if}
                {if $unpaid_workorders[u].status == '2'}{t}WORKORDER_STATUS_2{/t}{/if}
                {if $unpaid_workorders[u].status == '3'}{t}WORKORDER_STATUS_3{/t}{/if}
                {if $unpaid_workorders[u].status == '6'}{t}WORKORDER_STATUS_6{/t}{/if}
                {if $unpaid_workorders[u].status == '7'}{t}WORKORDER_STATUS_7{/t}{/if}
                {if $unpaid_workorders[u].status == '8'}{t}WORKORDER_STATUS_8{/t}{/if}
                {if $unpaid_workorders[u].status == '9'}{t}WORKORDER_STATUS_9{/t}{/if}
                {if $unpaid_workorders[u].status == '10'}{t}WORKORDER_STATUS_10{/t}{/if}
            </td>
