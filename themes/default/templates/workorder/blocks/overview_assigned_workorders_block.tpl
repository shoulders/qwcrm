    ...
    {section name=a loop=$assigned_workorders}    
        <tr onmouseover="this.className='row2';" onmouseout="this.className='row1';" onDblClick="window.location='index.php?page=workorder:details&workorder_id={$assigned_workorders[a].WORK_ORDER_ID}&customer_id={$assigned_workorders[a].CUSTOMER_ID}';" class="row1">
            ...
            <td class="olotd4" align="center">
                {if $assigned_workorders[a].WORK_ORDER_STATUS == '1'}{t}Created{/t}{/if}
                {if $assigned_workorders[a].WORK_ORDER_STATUS == '2'}{t}Assigned{/t}{/if}
                {if $assigned_workorders[a].WORK_ORDER_STATUS == '3'}{t}Waiting For Parts{/t}{/if}
                {if $assigned_workorders[a].WORK_ORDER_STATUS == '6'}{t}Closed{/t}{/if}
                {if $assigned_workorders[a].WORK_ORDER_STATUS == '7'}{t}Waiting For Payment{/t}{/if}
                {if $assigned_workorders[a].WORK_ORDER_STATUS == '8'}{t}Payment Made{/t}{/if}
                {if $assigned_workorders[a].WORK_ORDER_STATUS == '9'}{t}Pending{/t}{/if}
                {if $assigned_workorders[a].WORK_ORDER_STATUS == '10'}{t}Open{/t}{/if}
            </td>
