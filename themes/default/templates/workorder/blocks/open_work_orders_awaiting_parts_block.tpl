<!-- open_work_orders_awaiting_parts_block.tpl - Display Awaiting Parts (Work Orders - Open Page) -->
    ...
    <tr onmouseover="this.className='row2';" onmouseout="this.className='row1';" onDblClick="window.location='?page=workorder:details&wo_id={$awaiting.WORK_ORDER_ID}&customer_id={$awaiting.CUSTOMER_ID}&page_title={$translate_workorder_page_title} {$awaiting.WORK_ORDER_ID}';" class="row1">
        ...
        <td class="olotd4" nowrap>
            <img src="{$theme_images_dir}icons/16x16/view.gif" border="0"
                 onMouseOver="ddrivetip('<b><center>{$translate_workorder_closed_contact_info_title}</b></center><hr><b>{$translate_workorder_phone}: </b>{$awaiting.CUSTOMER_PHONE}<br> <b>{$translate_workorder_work}: </b>{$awaiting.CUSTOMER_WORK_PHONE}<br><b>{$translate_workorder_mobile}: </b>{$awaiting.CUSTOMER_MOBILE_PHONE}<br><b>{$translate_workorder_address}</b><br>{$awaiting.CUSTOMER_ADDRESS}<br>{$awaiting.CUSTOMER_CITY}, {$awaiting.CUSTOMER_STATE}<br>{$awaiting.CUSTOMER_ZIP}');"
        ...
        {else}
            <img src="{$theme_images_dir}icons/16x16/view.gif" border="0"
                 onMouseOver="ddrivetip('<center><b>{$translate_workorder_closed_contact_info_title}</b></center><hr><b>{$translate_workorder_work}: </b>{$awaiting.EMPLOYEE_WORK_PHONE}<br><b>{$translate_workorder_mobile}: </b>{$awaiting.EMPLOYEE_MOBILE_PHONE}<br><b>{$translate_workorder_home}: </b>{$awaiting.EMPLOYEE_HOME_PHONE}');"
                 onMouseOut="hideddrivetip();"> 
        ...
        <td class="olotd4" align="center" nowrap>
            <a href="?page=workorder:print&wo_id={$awaiting.WORK_ORDER_ID}&customer_id={$awaiting.CUSTOMER_ID}&page_title=$translate_workorder_print_title} {$awaiting.WORK_ORDER_ID}&escape=1">
