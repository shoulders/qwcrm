            ...
            <table border="0" cellpadding="4" cellspacing="0">
                ...
                <tr>                                    
                    {if $single_workorder[i].EMPLOYEE_DISPLAY_NAME != ''}                            
                        <td><b>{$translate_workorder_closed_by}:</b>{$single_workorder[i].EMPLOYEE_DISPLAY_NAME} on <b>{$translate_workorder_date}: </b>{$workorder_resolution[r].WORK_ORDER_CLOSE_DATE|date_format:$date_format}</td>                                                       
                    {/if}
                </tr>
                <tr>
                    ...
                    {if $workorder_notes[b].WORK_ORDER_NOTES_DESCRIPTION != ''} 
                        <tr>
                            <td>
                                <p>
                                    ------------------------------------------------------------------------------------<br>
                                    {$translate_workorder_print_this_note_was_created_by} <b>{$translate_workorder_technician}: </b>{$workorder_notes[b].EMPLOYEE_DISPLAY_NAME} {$translate_workorder_on} <b>{$translate_workorder_date}: </b> {$workorder_notes[b].WORK_ORDER_NOTES_DATE|date_format:"$date_format %R"}
                                </p>
                ...
                <tr>
                    ...
                    <td>
                        {section name=e loop=$workorder_schedule}
                            <b>{$translate_workorder_start_time}:</b> {$workorder_schedules[e].SCHEDULE_START|date_format:"$date_format %R"}<br>
                            <b>{$translate_workorder_end_time}:</b> {$workorder_schedules[e].SCHEDULE_END|date_format:"$date_format %R"}<br>
