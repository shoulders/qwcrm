...
function display_invoices($db, $direction = 'DESC', $use_pages = false, $page_no = '1', $records_per_page = '25', $search_term = null, $search_category = null, $status = null, $employee_id = null, $customer_id = null)
{
    ...
    if ($search_term != null) {
        $whereTheseRecords .= " AND ".PRFX."invoice.$search_category LIKE '%$search_term%'";
    }
    ...
    if ($employee_id != null) {
        $whereTheseRecords .= " AND ".PRFX."invoice.employee_id=".$db->qstr($employee_id);
    }

    // Restrict by Customer
    if ($customer_id != null) {
        $whereTheseRecords .= " AND ".PRFX."invoice.customer_id=".$db->qstr($customer_id);
    }
...
function get_invoice_prefill_items($db, $type = null, $status = null)
{
    ...
    // filter by type
    if ($type) {
        $sql .= " AND type=".$db->qstr($type);
    }
    
    // filter by status
    if ($status) {
        $sql .= " AND active=".$db->qstr($status);
    }
