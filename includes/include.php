...
function check_acl($db, $login_usergroup_id, $module, $page_tpl)
{
    ...
    // If installing
    if (defined('QWCRM_SETUP') && (QWCRM_SETUP == 'install' || QWCRM_SETUP == 'upgrade')) {
        return true;
    }
...
function write_record_to_activity_log($record, $employee_id = null, $customer_id = null, $workorder_id = null, $invoice_id = null)
{
    ...
    if (QFactory::getConfig()->get('qwcrm_activity_log') != true) {
        return;
    }
    
    // Use any supplied IDs instead of $GLOBALS[] counterpart
    if (!$employee_id) {
        $employee_id  = $GLOBALS['employee_id'];
    }
    if (!$customer_id) {
        $customer_id  = $GLOBALS['customer_id'];
    }
    if (!$workorder_id) {
        $workorder_id = $GLOBALS['workorder_id'];
    }
    if (!$invoice_id) {
